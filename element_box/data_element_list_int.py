...
class DataElementListInt(DataElement):
    def __init__(self,
                 name: str,
                 optionality: DataElementOptionality,
                 box_name: str,
                 validation_function: Callable[[Any], None] = None):
        super().__init__(name, DataElementType.LIST_INT, optionality, box_name, validation_function)

    def set_value_from_string(self, value: str):
        try:
            self.value = [int(e) for e in value.split(",")]
        except Exception as e:
            raise create_kecsoft_exception(f"Couldn't convert string <{value}> to integer",
